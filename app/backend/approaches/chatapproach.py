...
class ChatApproach(Approach, ABC):
    ...
    query_prompt_template = """以下は、これまでの会話の履歴と、ナレッジベースで検索して答える必要のあるユーザーからの新しい質問である。
    100以上のドキュメントを含むAzure AI Searchインデックスにアクセスできます。
    会話と新しい質問に基づいて検索クエリを生成します。
    引用元のファイル名やドキュメント名（info.txtやdoc.pdfなど）を検索クエリに含めないでください。
    検索キーワードに[]や<<>>内のテキストを含めない。
    '+'のような特殊文字は含めないでください。
    質問が英語でない場合は、検索クエリを生成する前に質問を英語に翻訳してください。
    検索クエリを生成できない場合は、数字の0だけを返してください。
    """
